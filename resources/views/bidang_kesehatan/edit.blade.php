...
<div class="container">
    ...
    <form action="{{ route('bidang_kesehatan.update', $item->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-3">
    ...
    @error('bulan') <small class="text-danger">{{ $message }}</small> @enderror
        ...
        <div class="mb-3"><label>Cakupan ASI Eksklusif (%)</label><input name="cakupan_asi_eksklusif" value="{{ old('cakupan_asi_eksklusif', $item->cakupan_asi_eksklusif) }}" class="form-control" step="0.01" type="number" min="0" max="100"></div>
